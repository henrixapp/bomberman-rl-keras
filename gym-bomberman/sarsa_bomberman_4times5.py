...
sarsa = SARSAAgent(model=model, nb_actions=nb_actions, policy=policy, gamma=0.99, nb_steps_warmup=5000, train_interval=4, delta_clip=1.)

sarsa.compile(Adam(lr=.00025), metrics=['mae'])

if args.mode == 'train':
    # Okay, now it's time to learn something! We capture the interrupt exception so that training
    # can be prematurely aborted. Notice that now you can use the built-in Keras callbacks!
    weights_filename = 'sarsa_{}without_det12m_weights.h5f'.format(args.env_name)
    checkpoint_weights_filename = 'sarsa_' + args.env_name + 'without_det12m_weights_{step}.h5f'
    log_filename = 'sarsa_{}_log.json'.format(args.env_name)
    callbacks = [ModelIntervalCheckpoint(checkpoint_weights_filename, interval=250000)]
    callbacks += [FileLogger(log_filename, interval=1000)]
    callbacks +=[keras.callbacks.TensorBoard(log_dir='./Graph', histogram_freq=0,  
          write_graph=True, write_images=True)]
    sarsa.fit(env, callbacks=callbacks, nb_steps=12500000, log_interval=100000,visualize=False)
    ...
    sarsa.test(env, nb_episodes=4, visualize=True)
